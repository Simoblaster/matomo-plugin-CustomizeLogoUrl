...
var CustomizeLogoUrl = (function() {
    ...
    var self = {};
    var config = {
        logoUrl: 'https://www.matomo.org',  // Default fallback
    ...
    self.loadConfiguration = function() {
        ...
        if (typeof window.CustomizeLogoUrlConfig !== 'undefined') {
            ...
            console.log('CustomizeLogoUrl: Configuration loaded from window.CustomizeLogoUrlConfig');
            return;
        }
        ...
        if (typeof $ !== 'undefined') {
            $.ajax({
                ...
                success: function(data) {
                    console.log('CustomizeLogoUrl: Configuration received from server:', data);
                    self.configure(data);
    ...
    self.modifyLogoLink = function() {
        ...
        for (var i = 0; i < selectors.length; i++) {
            ...
            if (logoElement) {
                console.log('CustomizeLogoUrl: Logo found with selector:', selectors[i]);
                break;
            }
        }
        ...
        if (logoElement) {
            ...
            console.log('CustomizeLogoUrl: Logo link modified from "' + 
                       logoElement.getAttribute('data-original-href') + '" to "' + newUrl + '"' +
                       (config.openInNewTab ? ' (new window)' : ''));
                       
        } else {
            console.log('CustomizeLogoUrl: Logo not found with any selector, retrying in 1 second...');
    ...
    self.restoreOriginalLink = function() {
        var logoElement = document.querySelector('[data-domaccessplugin-modified="true"]');
